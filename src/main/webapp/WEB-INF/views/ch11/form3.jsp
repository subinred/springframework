...
<div class="card m-2">
	...
	<div class="card-body">
		...
		<form:form modelAttribute="member" method="post" action="form3" class="mt-3">
			<div>
				<form:checkboxes items="${languageList}" path="mlanguage" cssClass="mr-2" delimiter="&nbsp; | &nbsp;"/>
			</div>
			<button class="btn btn-info btn-sm">제출</button>
		</form:form>
		...
		<form:form modelAttribute="member" method="post" action="form3" class="mt-3">
			<div>
				<form:checkboxes items="${skillList}" path="mskill" 
								 ...
								 cssClass="mr-2" delimiter="&nbsp; | &nbsp;"/> <!-- 파이프 기호 넣기 싫으면 빼도 돼! -->
								 <!-- cssStyle="width:50px;height;=red" 추가하면 체크박스 스타일 변경 -->
			</div>
			<button class="btn btn-info btn-sm">제출</button>
		</form:form>
	</div>
